...
namespace Rock.Workflow.Action.CheckIn
{
    ...
    /// </summary>
    [ActionCategory( "com_bricksandmortarstudio > Check-In" )]
    [Description( "Loads the groups available for each selected (or optionally all) location(s), while ignoring any checkin rules" )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "Load Groups While Ignoring Checkin Rules" )]
    [GroupTypeField( "Group Type To Ignore Checkin Rules For", "", false, Rock.SystemGuid.GroupType.GROUPTYPE_SERVING_TEAM, key: "IgnoredGroupType" )]
    ...
    public class LoadGroupsIgnoringCheckinRule : CheckInActionComponent
    {
        /// <summary>
                        ...
                        foreach ( var groupType in person.GetGroupTypes( !loadAll ) )
                        {
                            ...
                            if ( kioskGroupType != null )
                            {
                                foreach ( var kioskGroup in kioskGroupType.KioskGroups.Where( g => g.IsCheckInActive ) )
                                {
                                    ...
                                    Guid ignoredGroupTypeGuid = GetActionAttributeValue( action, "IgnoredGroupType" ).AsGuid();

                                    if ( groupType.GroupType.Guid != ignoredGroupTypeGuid )
                                    {
